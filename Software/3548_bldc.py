...
R = 0.077
m.setpar('motor.conf1.Lambda_m', 0.00135)
...
m.tracebg()
m.setpar('motor.state1.Iq_offset_SP',5.0)
time.sleep(0.1)
...
m.setpar('s1.hfi_gain_int2', 8*2*pi) # 5*2*pi
...
m.setpar('motor.state1.Iq_offset_SP',2.0)
...
plt.figure()
plt.title('Shaft velocity')
plt.plot(df.index,velocity,label='exp(Velocity/max_velocity) (rad/sec)')
plt.legend()
plt.show()

plt.figure()
plt.title('Measured Current')
plt.plot(df.index,Id,label='Id (A)')
plt.plot(df.index,Iq,label='Iq (A)')
plt.legend()
plt.show()
...
for hfi_v in [ 0.1, 0.5, 1.0, 1.5, 2.0]:
  m.setpar('s1.hfi_V', hfi_v)
